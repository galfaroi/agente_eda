#!/usr/bin/env python3
"""
Test runner for hard RAG agent tests using executor.py
Enforces Python code responses using OpenROAD Python API for complex workflows
"""
...
class TestExecutorWithDifficulty(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        print("\n" + "="*80)
        print("Setting up VLSI RAG Pipeline for Hard Python API Tests...")
        print("="*80)
    ...
    def test_python_queries(self):
        """Test all Python RAG queries for hard difficulty"""
        ...
        for i, test_case in enumerate(ACTIVE_TEST_PAIRS, 1):
            with self.subTest(test=i, difficulty="hard"):
                print(f"\n{'='*60}")
                ...
                print(f"HARD PYTHON TEST {i}: {description}")
                print(f"{'='*60}")
                ...
                if code:
                    ...
                    if language.lower() != "python":
                        ...
                        print("The RAG should generate Python code for complex workflows, not TCL!")
                    ...
                    if not is_valid:
                        ...
                        for issue in issues:
                            ...
                            elif issue.startswith("missing_complex_patterns"):
                                print(f"  - Missing complex workflow patterns: {issue}")
                            elif issue.startswith("problematic_api_usage"):
                                print(f"  - Using problematic API patterns: {issue}")
                                print("    Hint: Use proper OpenROAD workflow APIs like design.getFloorplan(), design.getReplace(), etc.")
                            elif issue == "contains_shell_commands":
                                print("  - Contains shell commands instead of Python API calls")
                            elif issue == "insufficient_workflow_complexity":
                                print("  - Hard tests require multi-step workflows with proper API usage")
                        ...
                        elif any("insufficient_workflow_complexity" in str(issue) for issue in issues):
                            print("⚠️  Executing anyway to test workflow...")
                        else:
                    ...
                    if not test_case.get('skip_execution', False):
                        print(f"\n🐍 Executing Hard Python code with OpenROAD...")
                        ...
                        # Provide specific guidance for common errors in hard tests
                        ...
                        if "design.__init__() missing 1 required positional argument" in stderr_lower:
                            ...
                            print("   Try using: tech = openroad.Tech(); design = openroad.Design(tech)")
                        elif "attributeerror" in stderr_lower and "has no attribute" in stderr_lower:
                            print("\n💡 GUIDANCE: API method doesn't exist or object not initialized")
                            print("   Check workflow: Tech() -> Design(tech) -> load files -> get modules")
                        elif "no such file or directory" in stderr_lower:
                            print("\n💡 GUIDANCE: File paths in hard tests may not exist")
                            print("   Focus on API structure rather than file loading for validation")
                        elif "floorplan not initialized" in stderr_lower:
                            print("\n💡 GUIDANCE: Floorplan must be initialized before use")
                            print("   Use: floorplan.initFloorplan() before other floorplan operations")
                    else:
                        print(f"\n⏭️  Skipping execution (marked as skip_execution)")
                    ...
                    # Check execution success for complex workflows
                    if not test_case.get('skip_execution', False):
                        if result['success']:
                            print(f"✅ Hard Python code executed successfully")
                        else:
                            print(f"⚠️  Hard Python code failed but provided learning information")
                    else:
                        print(f"✅ Hard Python code structure validated successfully")
                    ...
                    if "python" in response_lower and "openroad" in response_lower:
                        ...
                        print(f"❌ Response should mention Python API usage for complex workflows")
...
def run_single_test(test_index, difficulty_pairs):
    ...
    if 1 <= test_index <= len(difficulty_pairs):
        ...
        description = test_case.get('description', f"Query: {test_case['query'][:50]}...")
        print(f"Running single Hard Python test: {description}")
        if 'note' in test_case: # Check if 'note' exists
            print(f"Note: {test_case['note']}")
        ...
        if code:
            ...
            if language.lower() == "python":
                ...
                is_valid, issues, message = validate_python_code(code, test_case)
                print(f"Hard Python validation: {is_valid}, issues: {issues}")
                
                if not test_case.get('skip_execution', False):
                    result = execute_openroad_code(code, language)
                    print(f"Execution result: {result}")
                else:
                    print("Skipping execution (marked as skip_execution)")
            else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run VLSI RAG agent tests with hard difficulty.")
    parser.add_argument(
        '-d', '--difficulty', 
        type=str, 
        default='hard', 
        choices=['easy', 'medium', 'hard'], 
        help="Difficulty level of tests to run (default: hard)"
    )
    parser.add_argument(
        'test_index', 
        nargs='?', 
    ...
    if args.difficulty == 'easy':
        ...
        # Use easy validation for medium tests
        try:
            from test_easy_rag_pairs import validate_python_code as imported_validator
        ...
        from test_hard_rag_pairs import HARD_PYTHON_TEST_PAIRS as SELECTED_TEST_PAIRS
        from test_hard_rag_pairs import validate_python_code as imported_validator
        print(f"Running HARD tests from test_hard_rag_pairs.py (Found {len(SELECTED_TEST_PAIRS)} pairs)")
    else:
