...
def is_gui_command(code: str) -> bool:
    """Check if the code contains GUI-related commands."""
    gui_patterns = [
        "gui::show",
        "gui::hide", 
        "-gui",
    ...
    return any(pattern in code.lower() for pattern in gui_patterns)

def execute_gui_command(code: str) -> Dict[str, Any]:
    """Execute GUI commands in a special way - launch OpenROAD GUI with TCL script."""
    import os
    import signal
    
    print("🖥️  Detected GUI command - launching OpenROAD GUI with TCL script...")
    
    # Prepare TCL code for GUI execution
    # Remove the exit command since GUI should stay open
    gui_tcl_code = code.replace("exit", "").strip()
    
    # Add GUI-friendly TCL wrapper
    tcl_code = f"""
    # GUI Mode - Execute commands and keep GUI open
    set_cmd_errors 1
    set_error_propagate 0
    
    {gui_tcl_code}
    
    # Keep GUI open - don't exit
    puts "GUI commands executed. GUI will remain open for interaction."
    flush stdout
    flush stderr
    """
    ...
    try:
        # Launch OpenROAD GUI with the TCL script
        process = subprocess.Popen(
            ['openroad', '-gui', temp_path],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            preexec_fn=os.setsid  # Create new process group
        )
        
        # Give it a moment to start and execute initial commands
        import time
        time.sleep(3)
        ...
        if process.poll() is None:
            return {
                "stdout": f"OpenROAD GUI launched successfully with PID {process.pid}\n"
                         f"TCL script executed in GUI mode.\n"
                         f"GUI should be visible on your screen.\n"
                         f"To close GUI later, run: kill {process.pid}",
                "stderr": "",
                "exit_code": 0,
                "success": True,
                "language": "tcl",
            ...
            return {
                "stdout": stdout.decode() if stdout else "GUI process completed",
        ...
        return {
            "error": f"Failed to launch GUI with TCL: {str(e)}",
            "success": False,
            "language": "tcl"
        }
    finally:
...
def execute_tcl_with_openroad(code: str) -> Dict[str, Any]:
    ...
    # Check for GUI commands first
    if is_gui_command(code):
        return execute_gui_command(code)
